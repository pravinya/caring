...
class UserController extends Controller{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'logout' actions
				'actions'=>array('logout','uploadAvatar'),
				'users'=>array('@'),
			),
	...
	{      
           ...
           $model=new UserGroupsUser('registration');
             if(isset($_GET['fbUser']))
                     $fbUser = $_GET['fbUser'];
        //   if (Yii::app()->request->isAjaxRequest){
        //                echo 'yes ajax request';
             if (isset($_POST['UserGroupsUser'])) {
		
                 $this->performAjaxValidation($model, 'user-groups-registration-form');
                 $error = false;
		...
		/*foreach ($profiles as &$p) {
			...
			/*	foreach ($profiles as $p) {
					...
					$p['model']->save();
                                ...
                                Yii::app()->user->setFlash('user', Yii::t('userGroupsModule.general','Registration Completed. You can now login.Thank you.'));
				$array = array('signup' => 'success');
                             //   Yii::app()->user->setFlash('success', 'Registration Successfully completed.');
     ...
     //  }// ajax request
        else{  $this->layout = 'fancy_box_layout';}  
	...
	//print_r()	;var_dump($id);
		...
		$this->render('update',array('miscModel'=>$miscModel,'passModel'=>$passModel, 'profiles' => $profile_models));
	}
