...
class UserGroupsUser extends CActiveRecord
{
	...
	public function rules()
	{
		...
		$rules = array(
			...
			// rules for multiple scenarios
                        array('group_id', 'required', 'on' => array( 'registration')),
	...
	protected function afterSave()
	{
		...
		if (($this->scenario === 'admin' || $this->scenario === 'registration') && $this->status === self::WAITING_ACTIVATION) {
               ...
               // if ($this->status === self::FACEBOOK)     $mail = new UGMail($this, UGMail::FACEBOOK);
              $mail = new UGMail($this, UGMail::ACTIVATION);
	      $mail->send();
                        
                        /**  Chandre folder creation for image storage  **/                                       
                        $folder=Yii::getPathOfAlias('webroot.files.users').DIRECTORY_SEPARATOR;
                        if(!is_dir($folder.$this->id))
                                    mkdir($folder.$this->id,07777,true);
