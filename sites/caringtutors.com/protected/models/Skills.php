...
class Skills extends CActiveRecord
{
	...
	public static function getSkillNames($id){

           $criteria = new CDbCriteria;
           $criteria->with = array('cv_skills');
           $criteria->condition = "t.skill_id in (select skill_id from cv_skills where cv_id = '{$id}' )";
          $criteria->together = true;
          $skills = Skills::model()->findAll($criteria);//print_r($skills);Yii::app()->end();
                    $keys = array();$names=array();$ret = array();
		foreach($skills as $tag){
			$names[]=$tag->name;//Yii::app()->end();print_r($names);
			$keys[] = $tag->skill_id;
			}
	       $ret = array_fill_keys($keys,''); $i = 0;
	       foreach($ret as $key => $value) {
	           $ret[$key] = $names[$i]; $i++;
	        //array_push($ret,$n);	
	      	} // print_r($ret); Yii::app()->end();
	      /* foreach($ret as $key => $value) {
                            if($value == null) {
                                 unset($ret[$key]);
                                    }
		...
		//return self::array2string($names);
                        //print_r(array_unique($ret)); Yii::app()->end();		
                     return ( $ret);  
         }
	 
	public static function getCvSkillNames($id){
        ...
        public static function getCmpSkillNames($id){
