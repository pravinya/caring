...
class AdController extends Controller
{
	...
	{    $this->layout = 'application.views.layouts.adview';
		...
		if(!empty($lid)) {echo $lid; Yii::app()->end();}
		...
		if(!empty($cid) && is_numeric($cid)){  
			...
			$childs = $categoryInfo->children()->findAll();
			...
			if(!empty($childs)){
				...
				$inWhereArray = array();
				foreach ($childs as $k){  //echo $k->name;
					$inWhereArray[] = $k->id;
				}  //Yii::app()->end();
			}
			...
			//print_r($whereArray); Yii::app()->end();
			//set params to pager
			$pagerParams['cid'] = $cid;
			...
			$this->view->category_title = $categoryInfo->name;
		}
