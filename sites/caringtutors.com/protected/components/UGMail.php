...
class UGMail {
	...
	protected function extractUserData(UserGroupsUser $model)
	{
		...
		$link = 'http://'.$_SERVER['HTTP_HOST'].Yii::app()->homeUrl.'dashboard/public/activate';
...
/**
 * the mail message that will be sent to admin(notifier) that have been invited to join the application
 * @author Nicola Puddu
 * @package userGroups
 */
class UGMailNotification implements UGMailMessage {
	...
	public function mailSubject($data)
	{
		return Yii::t('userGroupsModule.mail', 'registered to {website}', $data);
	}
 ...
 * @package userGroups
 */
class UGMailFacebook implements UGMailMessage {
	...
	public function mailSubject($data)
	{
		return Yii::t('userGroupsModule.mail', 'registered via facebook to {website}', $data);
	}
	...
	public function mailBody($data)
	{
		return Yii::app()->controller->renderPartial('//mail/facebook', array(
			'data' => $data
			), true);
	}
	...
	public function mailSuccess($data)
	{
		return Yii::t('userGroupsModule.general','An email is sent to the address {email}', $data);
	}
