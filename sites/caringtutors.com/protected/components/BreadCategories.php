...
//        public $tagName = "";
	...
	{  	
             ...
             $model_class = ucfirst($controller->getId());
                 ...
                 if($model_class === 'Ad'){
                               ...
                               if(isset($_GET['id']))   {
                                     ...
                                     $this->category = $prof->category_id;
                            ...
                            foreach($this->roots as $rt) { 
                                if($rt->isRoot())
                                        ...
                                        array('ad/siteMap',"category"=>$rt->id,"map"=>$rt->name);
                                       //   CHtml::link($rt->name  ,Yii::app()->createUrl('profiles/siteMap',array("category"=>$rt->id,"map"=>$rt->name)));    
                                 else  
                                      $this->bcrumbs[$rt->name] = //DCUtil::getSeoTitle($rt->name);
                 ...
                 foreach($this->getCategories($rt->id) as $child) {
                                      $url = "";                    
                   if($child->isRoot())  {
                   $url = Yii::app()->createUrl('ad/siteMap',array("category"=>$child->id,"map"=>$child->name)); 
                     }
                   else{            
	  ...
	  if($model_class === 'Ad')  //$this->controller->breadcrumbs = $this->bcrumbs;
