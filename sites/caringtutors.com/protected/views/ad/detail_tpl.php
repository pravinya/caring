...
<?if(isset($this->view->adInfo) && !empty($this->view->adInfo)){
	...
	$ad = $this->view->adInfo;
	    ...
	    $this->pageTitle = $ad->ad_title;
	    echo '<div class="box_title" style="font-weight:normal;"><h1 style="margin-bottom:0px;">';
	    echo stripslashes($ad->ad_title);
	    echo '</h1></div>';
	    ?>
...
<div style="width: 515px;border: 1px solid #CCC;border-radius: 3px 3px 3px 3px;float: left;">
...
?> 
    ...
    <div style="line-height:18px; font-size:12px;">
		...
		<?}?>
		<?if(!empty($ad->ad_price)){?><?= '<i class="icon-inr"></i>' ;?>
			<?=Yii::t('detail_page', 'Price')?>: <b><?=$ad->ad_price?> </b><br />
		<?}?>
		<?if(!empty($ad->ad_link)){?>
